# MongoDB Notes

## Quick Links
- [CRUD Operations](https://www.mongodb.com/docs/drivers/csharp/current/fundamentals/crud/)
- [Operations with Builders](https://www.mongodb.com/docs/drivers/csharp/current/fundamentals/builders/)
- [Aggregation Reference](https://www.mongodb.com/docs/manual/reference/aggregation/)
- [Aggregation Video Playlist](https://www.youtube.com/playlist?list=PLRAV69dS1uWQ6CZCehxKy0rjkqhQ2Z88t)

## Table of Content
- [Aggregation Stages](AggregationStages.md)
- [Aggregation Operators](AggregationOperators.md)
- [Aggregation Examples](AggregationExamples.md)